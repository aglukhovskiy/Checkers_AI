...
class CheckersGame:
    ...
    def get_regular_moves(self):
        ...
        for row, col in self.pieces:
            ...
            if np.sign(piece) == self.current_player:
                reg_moves = self.get_regular_moves_for_piece(row, col)
                if reg_moves:
                    moves.extend(reg_moves)
    ...
    def available_moves(self):
        ...
        all_moves = []
        all_captures = self.get_capture_moves()

        # Если есть взятия, они обязательны
        if all_captures:
            # Для каждой последовательности взятий берем первый ход
